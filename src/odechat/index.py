...
if "currentprojectsinfo" not in st.session_state:
...
# ---------------------------------------
st.set_page_config(page_title="ODEchat",layout="wide")
...
with projects_panel:
	...
	with btnloc_createproject:
		if st.button("Create Project",on_click=do.createproject,
			args=["proj "+str(len(curprojects)+1),len(curprojects)+1]):
	...
	# if st.button("Update Project",on_click=do.updateallprojects,args=[st.session_state]):
	# 	st.toast(f"Project {st.session_state["name"]} Saved!")

	st.markdown("**Current Projects**")

	with st.container(height=200,border=True):
		for pinx,p in enumerate(st.session_state["currentprojectsinfo"]):
			...
			with openproj_btnloc:
				if st.button(p["name"],type="primary",width="stretch",key=f"btn-loadproj-{p['id']}"):
					...
					st.session_state["currentprojectsinfo"]=[]
					st.session_state["verify_eq_btnstate"]=False
					if len(st.session_state["chatdb"])==0:
						dialog_addequations()

			with saveproj_btnloc:
				if st.button("Save",icon=":material/save:",key=f"btn-saveproj-{p['id']}",
					on_click=do.saveproject,args=[p['name'],p['id'],st.session_state]):
	...
	# if st.button("Update All Projects",on_click=do.updateallprojects,args=[st.session_state],key="btn-updateproj"):
	# 	st.toast(f"Project {st.session_state["name"]} Saved!")
