		...
		for msg in chatmsg:
			...
			if msg["plotid"]>-1:
				...
				if msg["action"]=="runlsa":
					...
					curplotdata["ydata_low"]=st.session_state["datadb"][cursimid-1]["data"][plotproperties["ydata"][0]]
					curplotdata["ydata_high"]=st.session_state["datadb"][cursimid-1]["data"][plotproperties["ydata"][1]]
				...
				if plotproperties["plotstyle"][dinx]!="b":
...
"plotid":-1,"dataid":-1,"stateid":st.session_state["curmodelstate"],"contentid":-1}
	...
	if action=="runlsa":
		lsaplot={"simid":[len(st.session_state["datadb"])+1],"xdata":['parameters'],"ydata":['sens_lowval','sens_highval'],
...
def plot_dialog():
	...
	with options_col:
		with st.form("plot_formtmp"):
			...
			plotproperties={"simid":[],"axeslimits":[],'legend':[],"plotstyle":[],"xdata":[],"ydata":[],
	...
	with plot_col:
		if st.session_state["temp_parameters"]["actionparams"] is not None:
			for row in st.session_state["temp_parameters"]["actionparams"].itertuples():
				...
				plotproperties["simid"].append(simnumber)
...
projects_panel,chat_panel=st.columns([0.2,0.8])
	...
	if len(st.session_state["statedb"])>0: # Session has a model state
		...
		if st.session_state["temp_parameters"]["actionparams"] is not None:
			...
			# elif st.session_state["temp_parameters"]["chatmsg"]["action"]=="showmodel":
				...
				# For plot, actionparams are plotproperties [{"simid":[],"plotstyle":[],"xdata":[],"ydata":[]}]
				actionparamstr=" "
				# for inx,cursimid in enumerate(st.session_state["temp_parameters"]["actionparams"]["simid"]):
				#     actionparamstr+=f"({cursimid,st.session_state["temp_parameters"]["actionparams"]["xdata"][inx],st.session_state["temp_parameters"]["actionparams"]["ydata"][inx]}) "

				actionparamstr+=f"simid={st.session_state["temp_parameters"]["actionparams"]["simid"]} "
		...
		d[DLymph]/dt=(0.33*L*0.8*[DTight]+0.67*L*0.8*[DLeaky]-[DLymph]*L)/Vlymph"""
			...
			with st.container(height=200,border=True):
				...
				#     st.session_state["chatdb"].append(msg1)
					...
					# plotproperties={"simid":[2,4],"xdata":["Time","Time"],"ydata":["Dc","Dc"],
					...
					# plotproperties={"simid":[2, 4],"xdata":['Time', 'Time'],"ydata":['D_T_c', 'D_T_c'],
