...
def create_workflow_project():
	wftype=st.selectbox("Type",options=["Parameter exploration","PK/PD Visualization"])
	...
	if wftype=="PK/PD Visualization":
		...
		st.text("Ensure the file is in .csv form with columns 'Time_days', 'Concentration_nM', 'Dose_mpk'. At the moment only single dose calibration is supported. Mention different groups by 'Group' column.")
		...
		"runnca dataid=[1] time='Time' concentration='Concentration_nM' dose='Dose_mpk'",
		"note: Assuming the MW=150KDa, Animal WT=3Kg",
		f"calibrate dataid=1 time=Time concentration=Concentration_nM dose=Dose_mpk objective=Drugcc parameters=[V1,V2,CL,Q] bounds=[(1e-3,1),(1e-3,1),(1e-3,1),(1e-3,1)]",
		]
