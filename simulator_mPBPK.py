...
class PKmodel:
    ...
    def __init__(self,ent_param_list,ent_species_list,odes_reactions_list):
        ...
        for ode_inx,ode_reactions in enumerate(self.odes_reactions):
            ...
            for rinx,reaction in enumerate(ode_reactions):
                fullreaction+=f"self.reactions_states[{rcount+rinx}]*{reaction}+"
    ...
    def getModelObj(self):
        ...
        ModelObj["odes_reactions"]=self.odes_reactions
...
def load_session():
    ...
    for key,val in data.items():
        ...
        # return
            ...
            for modelstateobj in val:
                ...
                st.session_state[key].append(PKmodel(Param_ents,Species_ents,modelstateobj["odes_reactions"]))
    ...
    #         elif m["task"]=="note":
    #             st.subheader(f":blue[{m["content"]}]")
    #         elif m["task"]=="section":
    #             st.header(m["content"])
    #         elif m["task"]=="plot":
    #             xvar,yvar,simid=m["content"]
    #             print(f"simid={simid} len of simresults={len(st.session_state.simresults)}")
    #             print(st.session_state.simresults)
    #             print("---------------------")
    ...
    #         elif m["task"] in ["showmodel","showstate"]:
...
ROUTES={"showcontrols":[["view","show","list","controls","control"],"list controls: lists all the controls"],
    ...
    "importfromsimulator":[["import","parameters","parameter","simulator"],"import parameters: selects the current values from the simulator"]
}
...
if "modelstates" not in st.session_state:
    ...
    pnames=["Vp","VL","ISF","CLp","CLi","L","Kp","sigma_t","sigma_l","sigma_lym"]
    punits=["mL","mL","mL","mL/h","mL/h","mL/h","dimensionless","dimensionless","dimensionless","dimensionless"]
    pvalues=[0.85,2*0.85,4.35,0.005,0,0.12,0.8,0.95,0.42,0.2]
    pcomments=["Plasma volume","Lymph volume","Interstitial Fluid Volume","Clearance in Plasma","Clearance from tight/leaky",
    "Total Lymph flow","Kp is fraction of ISF available for mAb. Value=0.8 for IgG1 and 0.4 for IgG4",
    "Reflection coefficient for Tight","Reflection coefficient for leaky","Reflection coefficient for Lymph"]
    snames=["Dplasma","Dtight","Dleaky","Dlymph"]
    sunits=["mg/L","mg/L","mg/L","mg/L"]
    svalues=[188.23,0,0,0]

    # odes=["(Dlymph*L - Dplasma*0.33*L*(1-sigma_t) - Dplasma*0.67*L*(1-sigma_l) - CLp*Dplasma)/Vp",
    # "(0.33*L*(1-sigma_t)*Dplasma - 0.33*L*(1-sigma_lym)*Dtight)/(0.65*ISF*Kp)",
    # "(0.67*L*(1-sigma_l)*Dplasma - 0.67*L*(1-sigma_lym)*Dleaky)/(0.35*ISF*Kp)",
    # "(0.33*L*(1-sigma_lym)*Dtight + 0.67*L*(1-sigma_lym)*Dleaky - Dlymph*L)/VL"]

    odes_reactions=[["(Dlymph*L)/Vp","(-Dplasma*0.33*L*(1-sigma_t))/Vp","(-Dplasma*0.67*L*(1-sigma_l))/Vp",
    "(-CLp*Dplasma)/Vp"],["(0.33*L*(1-sigma_t)*Dplasma)/(0.65*ISF*Kp)","(-0.33*L*(1-sigma_lym)*Dtight)/(0.65*ISF*Kp)"],
    ["(0.67*L*(1-sigma_l)*Dplasma)/(0.35*ISF*Kp)","(-0.67*L*(1-sigma_lym)*Dleaky)/(0.35*ISF*Kp)"],
    ["(0.33*L*(1-sigma_lym)*Dtight)/VL","(0.67*L*(1-sigma_lym)*Dleaky)/VL","(-Dlymph*L)/VL"]]
    ...
    st.session_state.modelstates.append(PKmodel(Parameters,Species,odes_reactions))
...
if "simulator_modelstate" not in st.session_state:
    ...
    st.session_state.simulator_modelstate=PKmodel(n_P,n_S,n_ODEs)
...
if "simdf" not in st.session_state:
    ...
    st.session_state.simdf=st.session_state.simulator_modelstate.simulate(Dose(amount=188.23,unit="mg/L",interval=21,species='Dplasma'),simTime_days=84)

if "plotdf" not in st.session_state:
    cursimdata=st.session_state.simdf
    # cursimdata["legend"]="Dc"
    # cursimdata["ycol"]=cursimdata["dc"]
    cursimdata["legend"]="Dplasma (mg/L)"
    cursimdata["ycol"]=cursimdata["dplasma"]
...
def update_plotdata():
    ...
    curplotdata["ycol"]=curplotdata[st.session_state["yvar_select"].lower().split(" ")[0]]
...
def complete_plot_input():
    ...
    y_logscale=0
    if st.session_state["plot_yscale"]=="log":
        y_logscale=1
    ...
    st.session_state.messages[-1]={"id":curid,"ask":f"{userask.strip()} {plotvar_x},0 and {plotvar_y},{y_logscale}","task":routed["response"],
        "modelstate":len(st.session_state.modelstates)-1,
        "content":[plotvar_x,plotvar_y,0,y_logscale,len(st.session_state.simresults)-1],"show_current_msg":False}
...
def add_note():
    ...
    st.session_state.messages.append({"id":curid,"ask":"addnote","task":"note",
        "show_current_msg":True,"content":st.session_state["note_section_input"]})
    st.session_state["note_section_input"]=""

def add_section():
    st.session_state.interaction_counter+=1
    curid=st.session_state.interaction_counter
    st.session_state.messages.append({"id":curid,"ask":"addsection","task":"section",
        "show_current_msg":True,"content":st.session_state["note_section_input"]})
    st.session_state["note_section_input"]=""
...
def plottingdialog():
    ...
    with options_col:
        with st.form("plot_formtmp"):
            ...
            plotproperties["title"]=labels_cols[0].text_input("",placeholder="title",value="Mouse 7E3 mAb")
            plotproperties["xlabel"]=labels_cols[1].text_input("",placeholder="x label",value="Time (hours)")
            plotproperties["ylabel"]=labels_cols[2].text_input("",placeholder="y label",value="Concentration (mg/L)")

            limits_cols=st.columns(4)
            plotproperties["axeslimits"]=[limits_cols[0].number_input("xmin",value=0),
            limits_cols[1].number_input("xmax",value=1),limits_cols[2].number_input("ymin",value=0),
            limits_cols[3].number_input("ymax",value=1)]

            scale_cols=st.columns(2)
            plotproperties["yscale"]=scale_cols[0].selectbox("Y scale",options=["linear","log"])
            plotproperties["plotstyle"]=scale_cols[1].selectbox("Plot style",options=["line","scatter"])
            ...
            plotdata=pd.DataFrame({"Simulation":["Sim "+str(len(st.session_state.simresults))],
                "xdata":["time"],"ydata":[s1.name+" ("+s1.unit+")"],"Legend":[s1.name+" ("+s1.unit+")"]})
            ...
            st.session_state["edited_plotdata"]=st.data_editor(
                ...
                column_config={
                    ...
                    "xdata": st.column_config.SelectboxColumn(
                        "xdata",
                        options=["time"]+[s.name+" ("+s.unit+")" for s in st.session_state.simulator_modelstate.Species],
                        required=True,
                    ),
    ...
    with plot_col:
        ...
        # overallplotdata=pd.DataFrame({"time":[],"ycol":[],"legend":[]})
        plotproperties["plotdata"]=pd.DataFrame({"xdata":[],"ydata":[],"legend":[]})
        ...
        for row in st.session_state["edited_plotdata"].itertuples():
            ...
            curplotdata["ydata"]=st.session_state.simresults[simnumber-1]["simdata"][row.ydata.split(" ")[0].lower()]
            curplotdata["legend"]=row.Legend
            if row.Legend==None:
                curplotdata["legend"]=row.ydata

            # curplotdata=curplotdata[["time","ycol","legend"]]
            plotproperties["plotdata"]=pd.concat([plotproperties["plotdata"],curplotdata],axis=0)

        if len(plotproperties["plotdata"]["ydata"])>0:
            # overallplotdata=overallplotdata[overallplotdata["ycol"]>lim_ymin] # this is not needed since clip=True
            title = alt.TitleParams(plotproperties["title"], anchor='middle')
            chartobj_chatwindow=(
                alt.Chart(plotproperties["plotdata"],title=title)
                .mark_line(clip=True)
                .encode(x=alt.X("xdata:Q",title=plotproperties["xlabel"]).scale(domain=plotproperties["axeslimits"][:2]),
                    y=alt.Y("ydata:Q",title=plotproperties["ylabel"]).scale(type=plotproperties["yscale"],domain=plotproperties["axeslimits"][2:]),
                    color=alt.Color("legend",sort=None))
                )

            st.session_state.chat_plotproperties=plotproperties
            chartobj_chatwindow.configure_header(
                titleColor='black',
                titleFontSize=plotproperties["fontsize"],
                labelColor='black',
                labelFontSize=plotproperties["fontsize"]
            )

            st.altair_chart(chartobj_chatwindow,theme=None)


def update_equations(sel_reactions):
    st.session_state.simulator_modelstate.reactions_states=[1 if sel else 0 for sel in sel_reactions]
...
with st.form("druginputs",enter_to_submit=False):
    ...
    with m1:
        st.session_state.doseinterval=st.number_input("Dose interval",min_value=0.0,max_value=1000.0,value=21.0)

    with m2:
        st.session_state.simtime=st.number_input("Simulation Time",min_value=0.0,max_value=10000.0,value=84.0)
...
if runsim:
    ...
    for row in st.session_state.simulator_modelvals.itertuples(index=False):
        ...
        st.session_state.simulator_modelstate.update(UpdateParameters(parametername=row.Name,value=row.Value))
        if row.Name.lower()=='dplasma':
            doseamount=row.Value
            print("Dose amount is only taken from dplasma row")
    ...
    with plot_col:
        ...
        with species_col:
            yvar_select=st.selectbox("Plot",
                options=[s.name+" ("+s.unit+")" for s in st.session_state.simulator_modelstate.Species],
        ...
        chartobj=(
            ...
            .mark_line()
            .encode(x="time",y=alt.Y("ycol").scale(type=yscale,domain=[0.1,1000]),color=alt.Color("legend",sort=None))
            )

        st.session_state.chart=st.altair_chart(chartobj)


st.image("img/mPBPK.jpg",width=500)
